...
import { withDashboardCors, sameOriginFailure } from "../dashboard-http.js";
...
export const routeWorkspaceSettings = Effect.fn("WorkspaceSettings.route")(
  function* (
    request: Request,
    url: URL,
    ...
    if (url.pathname === "/api/v2/settings/workspace/policies" && request.method === "GET") {
      return Response.json(yield* operations.workspace("policies"));
    }
    if (url.pathname === "/api/v2/team" && request.method === "GET") {
      return Response.json(yield* operations.workspace("overview"));
    }
    if (url.pathname === "/api/v2/settings/workspace/members" && request.method === "GET") {
      return Response.json(yield* operations.workspace("members"));
    }
    if (url.pathname === "/api/v2/settings/workspace/invite" && request.method === "POST") {
      ...
      return Response.json(yield* operations.workspace("invite", body));
    }
    ...
    if (memberMatch && request.method === "POST") {
      ...
      if (memberMatch[2] === "remove") {
        return Response.json(yield* operations.workspace("remove", { user_id: userId }));
      }
      const body = yield* decodeBody(
        "workspace.role",
        request,
      ...
      return Response.json(
        yield* operations.workspace("role", { user_id: userId, role: body.role }),
      );
    }
    ...
    if (policyMatch && request.method === "POST") {
      ...
      if (policyMatch[2] === "reset") {
        return Response.json(
          yield* operations.workspace("policy_reset", { skill_set_id: skillSetId }),
        );
      }
      const body = yield* decodeBody(
