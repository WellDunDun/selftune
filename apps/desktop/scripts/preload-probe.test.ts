...
async function loadPreload(probeEnabled: boolean) {
  ...
  runInNewContext(stdout, {
    ...
    require: (name: string) => {
      ...
      return {
        contextBridge: {
          exposeInMainWorld: (key: string, value: object) => bridges.set(key, value),
        },
        ipcRenderer: {
...
test("sandboxed preload records one probe at document load without process.env", async () => {
  ...
  const probe: unknown = Reflect.get(bridge, "pendingWindowIpc");
  if (typeof probe !== "function") throw new Error("Missing probe.");
  const first: unknown = Reflect.apply(probe, bridge, []);
  const second: unknown = Reflect.apply(probe, bridge, []);
