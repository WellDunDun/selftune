...
interface UninstallResult {
  dryRun: boolean;
  service: { removed: boolean; details: string };
  credential: { removed: boolean; details: string };
...
export const removeRuntimeService = Effect.fn("SelfTuneUninstall.removeRuntimeService")(function* (
  ...
  const stoppedBeforeUnregister = yield* stopRuntime(configDir);
  if (backend.automated) yield* backend.uninstall(serviceDescriptor(configDir));
  const stoppedAfterUnregister = backend.automated ? yield* stopRuntime(configDir) : false;

  const actions: string[] = [];
  if (stoppedBeforeUnregister || stoppedAfterUnregister) {
    actions.push("Stopped the manifest-owned local runtime.");
  }
  if (backend.automated) actions.push(`Unregistered the ${backend.platform} service.`);
  return {
    removed: stoppedBeforeUnregister || stoppedAfterUnregister || backend.automated,
    details: actions.join(" ") || "No local runtime or supported OS service was found.",
  };
});

function removeRemoteCredential(dryRun: boolean): { removed: boolean; details: string } {
  const credential = storedRemoteLibraryCredential(SELFTUNE_CONFIG_DIR);
  if (!credential) return { removed: false, details: "No Remote Library credential found." };
  if (dryRun) {
    return {
      removed: false,
      details: `Would remove the Remote Library credential from ${credential.provider}.`,
    };
  }
  platformCredentialStore.delete(credential, SELFTUNE_CONFIG_DIR);
  return {
    removed: true,
    details: `Removed the Remote Library credential from ${credential.provider}.`,
  };
}
...
function isSelftuneHookEntry(entry: unknown): boolean {
  ...
  if (typeof entry.command === "string") {
    return isSelftuneCommand(entry.command);
  }
...
export async function uninstall(options: UninstallOptions): Promise<UninstallResult> {
  ...
  const service = await Effect.runPromise(removeRuntimeService(dryRun));
  const credential = removeRemoteCredential(dryRun);
  ...
  return { dryRun, service, credential, schedule, hooks, agents, logs, config, markers, npm };
}
...
Removes:
  ...
  2. Remote Library credential from the OS credential store
