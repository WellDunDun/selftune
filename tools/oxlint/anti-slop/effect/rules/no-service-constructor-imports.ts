...
// This syntax-only rule cannot inspect a factory's return type. Require an explicit
// Service suffix so ordinary factories and conventional Layer/Live recipes are not
// misreported as dependency-bearing Effect service constructors.
const SERVICE_CONSTRUCTOR_NAME = /^make[A-Z].*Service$/u;
...
export const noServiceConstructorImportsRule = defineRule({
	meta: {
		...
		docs: {
			description:
				"Disallow project-local runtime imports whose names follow the explicit make*Service constructor contract.",
		},
		messages: {
