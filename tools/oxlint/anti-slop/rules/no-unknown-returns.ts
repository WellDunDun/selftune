...
export const noUnknownReturnsRule = defineRule({
  meta: {
    ...
    docs: {
      description:
        "Disallow functions whose explicit return contract is unknown or Promise<unknown>, except reviewed runtime boundaries with a SAFETY-UNKNOWN justification.",
    },
    messages: {
      unknownReturn:
        "This function exposes `unknown` to its caller. Parse the value at its boundary, return a named domain type, or justify a genuine boundary with `SAFETY-UNKNOWN:`.",
    },
  },
  createOnce(context) {
    ...
    const checkReturnType = (node: FunctionWithReturnType) => {
      ...
      if (
        ...
        hasFunctionSafetyJustification(context.sourceCode, node, "SAFETY-UNKNOWN")
      )
        return;
