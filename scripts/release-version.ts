...
export function syncCoupledReleaseVersionFromDesktop(
  repoRoot = resolve(import.meta.dir, ".."),
): string {
  const desktopManifest = parsePackageManifest(resolve(repoRoot, "apps/desktop/package.json"));
  return setCoupledReleaseVersion(desktopManifest.version, repoRoot);
}
...
function run(): void {
  ...
  if (command === "sync-from-desktop") {
    const version = syncCoupledReleaseVersionFromDesktop();
    process.stdout.write(`Synced release packages to desktop v${version}\n`);
    return;
  }
  if (command !== "check") {
    throw new Error(
      "Usage: release-version.ts <check [--stable] [--tag-env NAME] [--output NAME] [--write-tag-env NAME]|compare-stable LEFT RIGHT|bump-patch|set VERSION|sync-from-desktop>",
    );
  }
