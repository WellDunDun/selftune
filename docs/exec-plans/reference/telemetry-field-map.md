...
| `skill_version_hash`  | required for causal claims | live hook hashes the complete installed skill package; replay stays empty unless source package bytes are preserved | derive only from the exact package used at invocation | derive only if source export preserves invoked package bytes | derive only from the exact package used at invocation | Unversioned records are descriptive evidence only |
...
Live `PostToolUse(Read)` capture records the concrete skill path and SHA-256 of
the file bytes. Replay uses the tool event's timestamp and latest preceding
actionable prompt, but does not hash the current filesystem for historical
events because that would misattribute later skill edits.
