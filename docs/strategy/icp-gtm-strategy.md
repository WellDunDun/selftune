...
**Implementation:** ✅ **SHIPPED** — 3-phase implementation complete. Phase 1: CLI `selftune badge --skill <name>` generates local SVGs. Phase 2: Dashboard server at `/badge/:skill` and `/report/:skill`. Phase 3: Hosted service at `selftune.dev` with SQLite aggregation from community contributions (`selftune contribute --submit`).
...
| 2 | Should badge service be self-hosted or SaaS? | Self-hosted / selftune.dev hosted | **selftune.dev hosted** (Fly.io + SQLite) | 2026-03-02 |
...
| 7 | Community signal pooling: centralized or P2P? | GitHub-based (current) / Central server / P2P | **Hybrid** — Central server (selftune.dev/api/submit) with GitHub fallback | 2026-03-02 |
...
| Badge service technical design | Hack #1 implementation | **Done** — 3-phase implementation (CLI + dashboard + hosted at selftune.dev) |
