...
export async function processPrompt(
  ...
  try {
    const { writeQueryToDb } = await import("@selftune/runtime/localdb/direct-write");
  ...
  try {
    ...
    if (signal) {
      const { writeImprovementSignalToDb } = await import("@selftune/runtime/localdb/direct-write");
