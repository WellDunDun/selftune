...
export function WizardSteps({
  ...
  return (
    <div className={cn("w-full", className)}>
      ...
      <span aria-hidden className="mb-2 grid select-none text-[13px] font-medium text-foreground">
        {steps.map((s, i) => (
          <motion.span
            key={s.id}
      ...
      <ol ref={listRef} aria-label={label} className="mb-4 flex list-none items-center gap-1 p-0">
        {steps.map((s, i) => {
          ...
          const tile = (
            <motion.span
              aria-hidden
              className={`grid size-7 place-items-center rounded-[8px] border text-[11.5px] font-medium tabular-nums shadow-sm transition-colors duration-150 ${
                done
            ...
            <li key={s.id} className="flex flex-1 items-center gap-1 last:flex-none">
              {i <= furthest ? (
                <button
                  ...
                  className="rounded-[8px] outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                >
                  {tile}
                </button>
              ) : (
      ...
      <div
        ...
        aria-label={position}
        style={{ height }}
        className="relative overflow-hidden rounded-[11px] border border-border bg-background shadow-sm outline-none transition-[border-color,box-shadow] duration-150 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
      >
        ...
        <AnimatePresence initial={false}>
          {isFirst ? null : (
            <motion.button
              ...
              className="h-9 rounded-[9px] border border-border bg-background px-3 text-[13px] font-medium text-foreground outline-none transition-[border-color,box-shadow] duration-150 hover:border-foreground/30 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
            >
              {backLabel}
            </motion.button>
          )}
        ...
        <AnimatePresence initial={false}>
          {complete ? null : (
            <motion.button
              ...
              transition={reduced ? { duration: 0 } : CROSSFADE}
              className="ml-auto grid h-9 place-items-center rounded-[9px] bg-primary px-3.5 text-[13px] font-medium text-primary-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
            >
