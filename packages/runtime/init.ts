...
import { findSelftunePackageRoot, resolveSelftuneCliEntrypoint } from "./package-root.js";
import type { SourceSyncRunner } from "./source-sync.js";
...
export async function cliMain(sourceSync?: SourceSyncRunner): Promise<void> {
  ...
  if (enableSync) {
    try {
      if (!sourceSync) {
        throw new Error("Source sync is not configured for this runtime.");
      }
      const syncResult = sourceSync();
  ...
  if (enableAutonomy) {
    try {
      const { installSchedule } = await import("./scheduling.js");
