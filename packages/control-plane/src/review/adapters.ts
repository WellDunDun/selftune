...
function candidateReview(
  summary: string,
  winner: CloudCandidateInput | null,
): RunReviewView["candidate"] {
  if (winner?.archiveUrl) {
    return {
      summary,
      diffText: winner.diffText,
      artifact: { label: "Download candidate package", href: winner.archiveUrl },
    };
  }
  return { summary, diffText: winner?.diffText ?? null };
}
...
export function adaptCloudImproveRun(input: CloudImproveReviewInput): RunReviewView {
  ...
  return {
    ...
    evidence: winner
      ...
      : [],
    candidate: candidateReview(summary, winner),
    decision: {
